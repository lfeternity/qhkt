...
const comfirePageInfoHandle = async () => {
  ...
  await confirmOrderInfo(params)
    ...
    .catch((error) => {
      ElMessage({
        message: error.response?.data?.msg || "获取结算信息失败，请稍后重试",
        type: 'error'
      });
    });
} 
...
const orderHandle = async () => {
  ...
  const selectedCouponIds = couponIds.value
    ? couponIds.value.split(",").filter(Boolean).map(Number)
    : []
  const params = {
    courseIds,
  ...
  await setOrder(params)
    ...
    .catch((error) => {
      ElMessage({
        message: error.response?.data?.msg || "提交订单失败，请稍后重试",
        type: 'error'
      });
    });
} 
