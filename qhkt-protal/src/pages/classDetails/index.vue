  ...
  <div class="classDetailsWrapper">
      ...
      <div class="container">
        ...
        <div class="topInfo fx">
          ...
          <div class="fx-1">
              ...
              <div class="fx">
                <div @click="collectionHandle" class="bt-wt bt-round marg-rt-15 ft-14" :class="{isCollection:isCollection}"> <i :class="{iconfont:true, 'zhy-btn_shoucang':!isCollection, 'zhy-btn_yishoucang':isCollection}"></i> {{ isCollection ? '已收藏' : '收藏' }}</div>
                <div class="bt-wt bt-round ft-14" @click="shareHandle"><weixin class="wx"></weixin> 分享</div>
              </div>
          </div>
        </div>
    ...
    <div class="DetailsContent container fx">
      ...
      <div class="ritCont">
        ...
        <!-- 猜你喜欢 -->
        <LikeCards v-if="LikeData.length" :data="LikeData"></LikeCards>
      </div>
    </div>
    <!-- 主体部分 - end -->
  </div>
...
import { getClassDetails, getClassTeachers, getClassList, getLikedStatus, putLiked } from "@/api/classDetails.js";
import { enrolledFreeCourse, putCarts } from "@/api/order.js";

import { getCourseLearning, getRecommendClassList } from "@/api/class.js";
...
const getCollectionStatus = async () => {
  try {
    const res = await getLikedStatus(detailsId.value)
    if (res.code === 200) {
      isCollection.value = (res.data || []).some(id => String(id) === String(detailsId.value))
    }
  } catch (error) {
...
const collectionHandle = async () => {
  if (!validation() || collectionLoading.value) {
    return
  }
  collectionLoading.value = true
  const liked = !isCollection.value
  try {
    const res = await putLiked({ bizId: detailsId.value, bizType: 'COURSE', liked })
    if (res.code !== 200) {
      throw new Error(res.msg || '收藏操作失败')
    }
    isCollection.value = liked
    ElMessage.success(liked ? '收藏成功' : '已取消收藏')
  } catch (error) {
    ElMessage.error(error.message || '收藏请求出错')
  } finally {
...
const shareHandle = async () => {
  const shareData = { title: baseDetailsData.value.name, url: window.location.href }
  try {
    if (navigator.share) {
...
const getRecommendClassData = async () => {
  try {
    const res = await getRecommendClassList('best')
    if (res.code === 200) {
      LikeData.value = (res.data || [])
        .filter(course => String(course.id) !== String(detailsId.value))
        .slice(0, 2)
    }
  } catch (error) {
    LikeData.value = []
...
const payHandle = () => {
  ...
  router.push({path: '/pay/settlement', query: {courseIds: detailsId.value}})
}
// 未登录处理购买、加入购物车点击问题
const validation = () => {
  if ( !isLogin()) {
