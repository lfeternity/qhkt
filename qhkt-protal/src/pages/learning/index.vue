...
const jumpToAgentCitation = async (event) => {
  const citation = event?.detail || {}
  const section = citation.sectionId ? sectionMap.value[citation.sectionId] : null
  if (!section) return
  await playHadle({ item: section, tp: '1' })
  const moment = Number(citation.startMoment)
  if (Number.isFinite(moment) && moment >= 0 && player.value) {
    player.value.currentTime(moment)
  }
}

onMounted(async () => {
  window.addEventListener('agent-jump-section', jumpToAgentCitation)
...
onUnmounted(() => {
  ...
  window.removeEventListener('agent-jump-section', jumpToAgentCitation)
})
